...
const Book = ({isRotating, setIsRotating, setCurrentStage, ...props}) => {
  ...
  const handlePointerMove = (e) => {
    ...
    if (isRotating) {
      bookRef.current.rotation.y += delta * 0.01 * Math.PI;
      lastX.current = clientX;
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'ArrowLeft') {
      ...
      bookRef.current.rotation.y += 0.005 * Math.PI;
  ...
  useFrame(() => {
    if (!isRotating) {
      const rotation = bookRef.current.rotation.y;
      const delta = rotationSpeed.current * dampingFactor;
  
      // Smoothly rotate the book
      bookRef.current.rotation.y += delta;
